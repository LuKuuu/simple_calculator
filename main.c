...
int mis(){
    ...
    float averageGradeCopy[3];

    for(int i = 0; i < 3; i++)
    {
        averageGradeCopy[i] = averageGrade[i];
    }
    ...
    for(int i=0;i<3-1;i++)
    {
        ...
        {
            if(averageGradeCopy[j]>averageGradeCopy[j+1])
            {
                temp=averageGradeCopy[j+1];
                averageGradeCopy[j+1]=averageGradeCopy[j];
                averageGradeCopy[j]=temp;
            }
        }

        printf("the ranking is in the order of");
